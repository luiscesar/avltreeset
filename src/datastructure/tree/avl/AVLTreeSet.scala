...
sealed abstract class AVLTreeSet[T<%Ordered[T]](var h: Int) {
  ...
  protected def balance(t: AVLTreeSet[T]) : AVLTreeSet[T] = t match {
    ...
    case NonEmptyAVLTreeSet(x,sf,l,r) if (sf.abs == 1) => t
    case NonEmptyAVLTreeSet(x,sf1,NonEmptyAVLTreeSet(y,sf2,t1,t2),r) if ( ((sf1 == -2) && (sf2 == -1)) ||
      ((sf1 == -2) && (sf2 == 0))) => t.rightRotation()
    case NonEmptyAVLTreeSet(x,sf1,l,NonEmptyAVLTreeSet(y,sf2,t1,t2)) if ( ((sf1 == 2) && (sf2 == 1)) ||
      ((sf1 == 2) && (sf2 == 0)) ) => t.leftRotation()
    case NonEmptyAVLTreeSet(x,sf1,NonEmptyAVLTreeSet(y,sf2,t1,NonEmptyAVLTreeSet(z,_,t2,t3)),r) if ( (sf1 == -2) && (sf2 == 1) ) => t.leftRightRotation()
    case NonEmptyAVLTreeSet(x,sf1,l,NonEmptyAVLTreeSet(y,sf2,NonEmptyAVLTreeSet(z,_,t1,t2),t3)) if ( (sf1 == 2) && (sf2 == -1) ) => t.rightLeftRotation()
  }
...
private case class NonEmptyAVLTreeSet[T<%Ordered[T]](v:T,private var sf:Int,left:AVLTreeSet[T],right:AVLTreeSet[T]) extends AVLTreeSet[T](1+AVLTreeSet.max(left.h,right.h)) {   
  ...
  def +(v:T): AVLTreeSet[T] = this match {
    case NonEmptyAVLTreeSet(x,sf,l,r) if (v < x) => {
      val t = l+v;
      val t1= new NonEmptyAVLTreeSet(x,r.h-t.h,t,r);
      balance(t1);
    }
    case NonEmptyAVLTreeSet(x,sf,l,r) if (v > x) => {
      val t = r+v;
      val t1 = new NonEmptyAVLTreeSet(x,t.h-l.h,l,t);
      balance(t1)}
    case _ => this
  }
  ...
  def -(v: T): AVLTreeSet[T] = this match {
    ...
    case NonEmptyAVLTreeSet(x,_,EmptyAVLTreeSet(),EmptyAVLTreeSet()) => new EmptyAVLTreeSet()
    case NonEmptyAVLTreeSet(x,_,EmptyAVLTreeSet(),r) => r
    case NonEmptyAVLTreeSet(x,_,l,EmptyAVLTreeSet()) => l
    case NonEmptyAVLTreeSet(x,_,l,r) => {
      var y = inorderPredecesor(l);
      val t=l-y;
      val t1 = new NonEmptyAVLTreeSet(y,r.h-t.h,t,r);
      balance(t1)
    }
  }
